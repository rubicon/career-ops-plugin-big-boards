...
/** Build the two location passes (DFW-default local + remote) for one
 *  searched title. Hardcoded shape, unchanged from the source engine;
 *  making the pass list itself configurable is a separate issue. */
  ...
  return [
    {
      ...
      input: {
        ...base,
        location: cfg.location || 'Dallas, TX',
        distance: cfg.distance || 75,
        remote_only: false,
      },
    },
    { label: 'remote', input: { ...base, remote_only: true } },
  ];
}
...
export async function scanApify(ctx) {
  ...
  const titles = Array.isArray(settings.titles) ? settings.titles.filter(Boolean) : [];
  if (titles.length === 0) {

...
/** A title passes when it matches >=1 positive AND 0 negatives. An empty or
 *  unset positive list means "no title filtering" (allow all), since
 *  title_filter narrows results already paid for at fetch time and must
 *  never be the reason every job gets dropped. Negatives still override
 *  positives (e.g. "Chief Medical Officer (CMO)" is rejected even though it
 *  matches the positive "CMO"), and an empty negative list blocks nothing. */
...
/** Keep jobs in the configured city list (empty if unset) plus remote; drop
 *  on-site jobs elsewhere. The actor's own location filter is unreliable, so
